...
int main() {
    ...
    do{
        ...
        switch(option){
            case 1: cout<<"\nElement a inserir: ";cin>>n;
                    try{
                        array.enqueueFront(n);
                        n=0;
                    }catch(const invalid_argument& e){
                        cout<<e.what()<<"\n"<<endl;// ArrayDeque està ple.
                    }; break;
            case 2: cout<<"\nElement a inserir: ";cin>>n;
                    try{
                        array.enqueueBack(n);
            ...
            case 3: try{array.dequeueFront();}catch(const invalid_argument& e){cout<<e.what()<<"\n"<<endl;};break;
            case 4: try{array.dequeueBack();}catch(const invalid_argument& e){cout<<e.what()<<"\n"<<endl;};break;
            case 5: try{
                        int front = array.getFront();
                        cout<<"\nPrimer element: "<<front<<"\n"<<endl;
                    }catch(const invalid_argument& e){
                        cout<<e.what()<<"\n"<<endl;// S'ha produit una excepció (el deque està buit)
                    };break;
            case 6: try{
                        int rear = array.getBack();
                        cout<<"\nÚltim element: "<<rear<<"\n"<<endl;
                    }catch(const invalid_argument& e){
                        cout<<e.what()<<"\n"<<endl;// S'ha produit una excepció (el deque està buit)
                    };break;
            case 7: try{cout<<"\nDeque: ";array.print();}catch(const invalid_argument& e){cout<<e.what()<<"\n"<<endl;};break;
        }
    }while(option!=8);
